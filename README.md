...
本地安装包：`artifacts/train-trip-v2-debug.apk`（构建产物，不随源码提交）。自行构建后可从 `app/build/outputs/apk/debug/app-debug.apk` 获取，将文件传到手机后点击安装。应用名“有票就出发”，包名 `cn.traintrip.app`，版本 `0.2.0`。这是本机调试签名的自用版本，尚未上架。
