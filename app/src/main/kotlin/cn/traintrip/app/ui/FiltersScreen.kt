        ...
        Column(verticalArrangement=Arrangement.spacedBy(8.dp)) {
            ...
            Row(Modifier.fillMaxWidth(),horizontalArrangement=Arrangement.spacedBy(6.dp)) {
                listOf(Triple("全天",0,1440),Triple("凌晨",0,360),Triple("早上",360,720),Triple("下午",720,1080),Triple("晚上",1080,1440)).forEach { (name,start,end) ->
                    Choice(name,f.startMinute==start && f.endMinute==end,{onUpdate(f.copy(startMinute=start,endMinute=end))},Modifier.weight(1f),contentPadding=PaddingValues(horizontal=4.dp,vertical=12.dp),maxLines=1)
                }
            }
