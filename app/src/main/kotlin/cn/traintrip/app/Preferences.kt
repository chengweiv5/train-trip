...
class Preferences(context: Context) {
    ...
    fun load(catalog: StationCatalog): SearchFilters = runCatching {
        ...
        val origin=j.optString("origin","0357").takeIf { it in catalog.byCity } ?: "0357"
        ...
        SearchFilters(origin,j.strings("stations").filter { catalog.byCode[it]?.cityId == origin }.toSet(),start,end,j.optInt("from",0),j.optInt("to",1440),
            j.strings("seats").mapNotNull { runCatching { SeatType.valueOf(it) }.getOrNull() }.toSet().ifEmpty { SeatType.entries.toSet() },
            j.optInt("people",1).coerceIn(1,20),j.optInt("max",0).takeIf { it>0 },j.strings("destinations").filter { it in catalog.byCity && it!=origin }.toSet().ifEmpty { catalog.initialDestinations(origin) })
    ...
    private fun defaults(c:StationCatalog)=SearchFilters(destinationCityIds=c.initialDestinations("0357"))
