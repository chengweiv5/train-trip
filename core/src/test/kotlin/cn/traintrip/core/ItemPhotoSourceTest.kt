...
class ItemPhotoSourceTest {
    ...
    @Test fun foodSearchSkipsExistingPlacesAndLabelsDishReferencePhotos()=runBlocking {
        MockWebServer().use { server ->
            val images = (1..3).map { index -> mapOf("Title" to "保定驴肉火烧成品实拍照片", "Url" to "https://www.hebei.gov.cn/food.html",
                ...
                mapOf("Title" to "驴肉火烧成品照片", "Url" to "https://www.hebei.gov.cn/food.html", "Image" to mapOf("Url" to photo(food,6).remoteUrl, "Width" to 900, "Height" to 600)))
            ...
            assertEquals(4,result.photos.size);assertTrue(result.photos.all { it.subject==food && it.description == "驴肉火烧 · 菜品参考图" })
