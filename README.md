...
v0.4.0 已实现筛选条件、真实余票查询、目的地城市汇总和车次详情。车次页支持手动刷新当前城市、复制所选车次，并直接打开铁路12306 App。打开 App 不强制查询、不自动复制、不弹二次确认，也不回退官网。
...
0.4.0 优化席别选中、底部摘要和复制入口，适配 320dp / 1.3 倍字体。局部刷新失败时保留旧车次和原始查询时间，展示未更新范围并支持重试；未安装铁路12306或打开失败时保留选择。验证记录见 [车次页与 App 跳转验收](docs/verification/2026-09-19-v0.4.0-implementation.md)。
