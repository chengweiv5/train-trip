...
- [x] **Step 1: 写并运行失败测试。**
  ...
  - 三项普通 Failure、一项 NotOnSale 后一项 Success，断言最后一项仍被调用。
...
- [x] **Step 3: UI 与取消验证。** 使用注入的离线 TicketSource 在模拟器上完成三项查询；第一项失败、后两项成功。断言待查为 0、显示两城和重试按钮，错误弹窗包含原错误；点击重试后只重新请求失败项并恢复完整结果。运行新 SearchFailureTest。
- [x] **Step 4: 构建与安装。** bump 版本；执行 `:core:test :app:assembleDebug :app:assembleDebugAndroidTest :app:lintDebug --offline`。先备份真机 APK 与 travel-filters.xml；核对签名后 adb install -r，回读版本、安装包哈希、启动状态与偏好哈希。
- [x] **Step 5: 记录与提交。** 保存测试/构建/安装摘要、更新 README、检查 diff，提交到 codex/query-failure-isolation。通过 punk-12 发送简短完成通知并确认发送结果。
