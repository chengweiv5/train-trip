...
class GuidePhotoSourceTest {
    ...
    private val guide=DestinationGuides.all.first().copy(cityId=city.id,name=city.name)
    @Test fun failedPrimaryUsesFallbackAndKeepsFailureVisible()=runBlocking {
        val photo=guide.photo!!;var calls=0
        ...
        assertEquals(1,calls);assertEquals(listOf(photo),result.photos);assertTrue(result.failed)
    }
    @Test fun fullPrimarySkipsPaidSearchAndCancellationNeverFallsBack()=runBlocking {
        ...
        val photos=(1..5).map { guide.photo!!.copy(assetName="a$it.jpg",remoteUrl="https://dimg.c-ctrip.com/$it.jpg") }
