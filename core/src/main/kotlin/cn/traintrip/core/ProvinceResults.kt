...
fun groupResults(catalog: StationCatalog, filters: SearchFilters, progress: SearchProgress?, byCount: Boolean = false, uncertain: Boolean = false): List<ProvinceResult> {
    val results=aggregate(progress?.trips.orEmpty(),filters,uncertain)
    ...
    return catalog.provinces.mapNotNull { province ->
        ...
        if(uncertain && cities.isEmpty() || !uncertain && scope.isEmpty() && cities.isEmpty()) return@mapNotNull null
        ...
        val status=when {
            ...
            cities.isEmpty() && !uncertain -> "该省暂无符合条件的票"
            else -> "所选范围查询完成"
        }
