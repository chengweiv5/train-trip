...
class Preferences(context: Context) {
    ...
    fun load(catalog: StationCatalog): SearchFilters = runCatching {
        ...
        val legacy=SearchFilters(origin,j.strings("stations"),start,end,j.optInt("from",0),j.optInt("to",1440),
        ...
        val periods=runCatching {
            j.optJSONArray("periods")?.let { a->(0 until a.length()).map { DeparturePeriod.valueOf(a.getString(it)) }.toSet() }
        }.getOrNull()
        val migrated=catalog.normalize(if(periods.isNullOrEmpty())legacy else legacy.withDeparturePeriods(periods))
    ...
    fun save(f:SearchFilters) {
        val j=JSONObject().put("origin",f.originCityId).put("stations",JSONArray(f.originStations.toList())).put("start",f.startDate.toString()).put("end",f.endDate.toString())
            ...
            .put("periods",JSONArray(f.departurePeriods.sortedBy { it.startMinute }.map { it.name }))
