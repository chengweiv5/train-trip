...
- [x] 根据实际读取的网页重新提炼五城景点、美食，保存标题、URL、核对日期与照片摘要清单。
- [x] 原图 EXIF 转正、RGB JPEG、最长边 1200，保留完整原图画面；既有 Image ContentScale.Crop 保持展示规则。
- [x] 使用 java.net.URI 校验外链：scheme in setOf("http", "https") && !host.isNullOrBlank() && userInfo == null。拒绝 javascript/file 与无 host 链接。
- [x] 验证可选许可：两字段同为 null，或名称非空且 URL 有效；其余均使损坏目录降级。
- [x] 更新既有核心测试，添加无许可成功、成对许可成功、半对许可与非法 URL 失败的行为用例；确认五城图像均可解码且正文来源为国内域名。
...
- [x] 移除固定正文 CC 入口，说明国内资料整理和玩法建议。
- [x] 图片展示 credit、个人离线浏览说明及原图页面入口；只有 licenseUrl != null 时显示许可按钮。
- [x] 调整来源点击测试，验证国内原文/图片出处 URL、无许可时按钮不存在，有许可样例仍正确打开。
- [x] 构建并在本任务隔离模拟器执行 DestinationGuideTest；保留四分页、返回、320dp/1.3 字体和短屏用例。
...
- [x] 新配图、来源弹窗、常规与大字体截图结构检查及目视/OCR，验证真实来源不混入旧许可。
- [x] 真机 FMR0224725012307 先备份 base.apk 与 travel-filters.xml，再 adb install -r；启动后回读 APK SHA256、包版本、筛选设置字节一致。
- [x] 写验收与回滚记录。

收尾：本地提交后通过 punk-12 发送完成通知，回执保存在私有验收目录；本轮不推送公开仓库。
